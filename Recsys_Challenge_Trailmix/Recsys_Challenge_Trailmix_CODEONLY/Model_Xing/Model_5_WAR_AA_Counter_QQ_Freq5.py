...
start = int(sys.argv[1]) * 2500

print ('Start Now 0620!~!! ~~~')
...
small_test_war = {}
for pl in list(WAR_raw.keys())[start : start + 2500]:
    small_test_war[pl] = []
    for t in WAR_raw[pl]:
        small_test_war[pl].append(track_index[t])
...
QQ = json.load(open('QQ_submit_2_15.json'))
...
with open('../WAR/0620/WAR_PREDICT_0620/Submission_0620_WAR_P_' + sys.argv[1] + '.json', 'w') as f:
	json.dump(R_combined, f)